...
function FormatDots(s)
  local result = s
    for match in string.gmatch(s, "(%d+%.%d+)") do
      local repl = string.gsub(match, '%.', "\\.", 1)
      result = string.gsub(result, match, repl, 1)
    end
    return result
end
...
function ComponentIsValid(component)
    ...
    for i, choice in ipairs(ComponentList) do if (choice.Text == component) then return true; end end
...
function Sync()
    ...
    if (not dir.get(directory)) then
        SetStatus(1, "Directory Doesn't Exist", "sync")
        return print("Could not Sync - Directory Does not Exist")
    end
    ...
    -- check for file
    if (not file) then
        SetStatus(1, "File Doesn't Exist", "sync")
        return print("Could not Sync - File Does not Exist")
    end
...
function UpdateList(data)
    ...
    local function PushValues(component, props)
        ...
        local ctl = FormatDots(props.Control)
        print(ctl)
        component[ctl].String = props.String
    end
    ...
    if (not data) then
        ...
        table.insert(Errors, {
            Color = "Red",
            Text = "Invalid JSON - Check Formatting"
        })
    ...
    for component, json_tbl in pairs(data) do
        ...
        if ComponentIsValid(component) then
                ...
                if err then
                    ...
                    for i, choice in ipairs(ComponentList) do
                        if (choice.Text == component) then
                            ComponentList[i]["Color"] = err:match("Control is read%-only") and "" or "Orange"
                        end
                    end
            ...
            table.insert(Errors, {
                Color = "Red",
                Text = string.format("Missing Component: '%s'", component)
            })
...
function Save()
    ...
    for i, choice in pairs(ComponentList) do Data = AddSelectedControls(i, choice, Data) end
    ...
    if (not file) then
        SetStatus(1, "File Doesn't Exist", "save")
        return print("File Doesn't Exist - Can't Save")
    end

    print(string.format("Saving Configuration:\n\n%s", rapidjson.encode(Data, {
        pretty = true
    })))
...
function AddSelectedControls(i, choice, tbl)
    ...
    if (not ComponentList[i].Controls) then
        return print(string.format("[No Controls Selected for Component '%s']", choice.Text))
    end
    ...
    for _, control in ipairs(ctrls) do
        if (control.Selected) then
            ...
            local ctl = FormatDots(control.Text)
            local controlString = Component.New(choice.Text)[ctl].String
            table.insert(tbl[choice.Text], {
...
function SelectAll(bool)
    ...
    for i, choice in ipairs(ComponentList) do
        ...
        ComponentList[i].Icon = ComponentList[i].Selected and utf8.char(10687) or utf8.char(9675)
        GetControlsOnComponent(choice.Text, i, {
            selectAll = bool
        })
    ...
    for i, choice in ipairs(ComponentList) do
        ...
        if (parent_component == choice.Text) then
            ...
            for id, control in ipairs(choice.Controls) do
                ...
                ComponentList[i].Controls[id].Icon = ComponentList[i].Controls[id].Selected and utf8.char(10687) or
                                                         utf8.char(9675)
...
Controls["components_list"].EventHandler = function(c)
    ...
    for i, choice in ipairs(ComponentList) do
        ...
        if (component == choice.Text) then
            ...
            ComponentList[i].Icon = ComponentList[i].Selected and utf8.char(10687) or utf8.char(9675)
            if (ComponentList[i].Selected) then
                GetControlsOnComponent(component, i)
            else
                ResetControlsList()
            end
        end
